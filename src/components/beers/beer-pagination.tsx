...
export const BeerPagination = ({
  ...
  const totalPages = Math.ceil(totalDocs / BEERS_PER_PAGE)
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const currentPage = Number(searchParams.get('page')) || 1

  const startPost = (currentPage - 1) * BEERS_PER_PAGE + 1
  const endPost = Math.min(currentPage * BEERS_PER_PAGE, totalDocs)
